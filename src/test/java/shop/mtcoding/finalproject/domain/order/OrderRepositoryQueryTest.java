...
public class OrderRepositoryQueryTest extends DummyEntity {
    ...
    public void 조회_test() throws Exception {
        // given
        FindOrderStatsReqDto reportReviewRespDtos = new FindOrderStatsReqDto();
        reportReviewRespDtos.setStoreId(1L);
        reportReviewRespDtos.setStartTime("");
        reportReviewRespDtos.setEndTime("");

        // when
        OrderStatsRespDto orderStatsRespDto = orderRepositoryQuery.findAllOrderStatsByStoreId(reportReviewRespDtos);

        // then
        Assertions.assertThat(orderStatsRespDto.getOrderCount()).isEqualTo("3");
        Assertions.assertThat(orderStatsRespDto.getOrderAmount()).isEqualTo("72000");
